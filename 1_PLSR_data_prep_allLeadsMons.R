...
for (wk_i in c('1_2')) {#c('2_3', '3_4')) {
  for (pred_mon in c(6:12)) {
    ...
    print(paste('processing bi-weekly period', wk_i, 'for predictand month', month.name[pred_mon],'...'))
    ...
    setwd(cfs_dir)
    file_hgt   = nc_open(paste0('cfsv2.', wk_i, '.z500_f.nc')) 
    file_q2m   = nc_open(paste0('cfsv2.', wk_i, '.q2m_f.nc'))
    file_prs   = nc_open(paste0('cfsv2.', wk_i, '.pressfc_f.nc'))
    file_slp   = nc_open(paste0('cfsv2.', wk_i, '.prmsl_f.nc'))
    file_pwt   = nc_open(paste0('cfsv2.', wk_i, '.pwat_f.nc'))
    file_uwnd  = nc_open(paste0('cfsv2.', wk_i, '.U_wnd850_f.nc'))
    file_vwnd  = nc_open(paste0('cfsv2.', wk_i, '.V_wnd850_f.nc'))
    file_sst   = nc_open(paste0('cfsv2.', wk_i, '.ocnsst_h.nc'))
    file_olr   = nc_open(paste0('cfsv2.', wk_i, '.ulwtoa_f.nc'))
    file_tmp   = nc_open(paste0('cfsv2.', wk_i, '.tmp2m_f.nc'))
    file_prt   = nc_open(paste0('cfsv2.', wk_i, '.prate_f.nc'))
    ...
    setwd(out_model_dir)
    rm(list=setdiff(ls(), c('data_out', 'file_data', 'l', 'file_lag_in', 'beg.time', 'wk_i', 'pred_mon')))
    save(data_out, file = file_data)
    
    
    ###### ====== Average lagged variables loop through time ====== ######
    # takes a long time (~20 mins)
    print(paste('lagging file ...', file_data, 'for wk', wk_i))
    print(paste('elapsed time ...')); print(Sys.time() - beg.time)
    ...
    for (itime in 1:Ntime) {
      ...
      t_ind = plyr::rbind.fill(t_ind, t_i)
    }
